...
public class ChatErrorHandler extends StompSubProtocolErrorHandler {
    ...
    private Message<byte[]> messageException(Message<byte[]> clientMessage, Throwable ex) {
        return errorMessage(HttpStatus.BAD_REQUEST);
    }

    //jwt 예외
    private Message<byte[]> jwtException(Message<byte[]> clientMessage, Throwable ex) {
        return errorMessage(HttpStatus.CONFLICT);
    }

    //메시지 생성
    private Message<byte[]> errorMessage(HttpStatus status) {
        String code = status.toString();

        StompHeaderAccessor accessor = StompHeaderAccessor.create(StompCommand.ERROR);

        accessor.setMessage(String.valueOf(status.value()));
